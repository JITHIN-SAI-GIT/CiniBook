...
export default function NearbyTheatresSection() {
  ...
  useEffect(() => {
    ...
    if (fetchKey === lastFetchKey.current && retryCount === 0) return;
    ...
    const load = async () => {
      ...
      try {
        ...
        // Show local theatres immediately if we have them
        if (localTheatres.length > 0) {
          localTheatres.sort((a, b) => (a.distance ?? 999) - (b.distance ?? 999));
          setTheatres([...localTheatres]);
          setLoading(false);
        }

        // 2. Get OSM theatres if we have GPS (Slow, run asynchronously)
        if (userLocation && !controller.signal.aborted) {
          // Add a 15-second safety timeout for OSM
          const timeoutPromise = new Promise((_, reject) => {
            timeoutId = setTimeout(() => reject(new Error('OSM Timeout')), 15000);
          });

          try {
            const osmPromise = getNearbyTheatresFromOSM(
              userLocation.lat,
              userLocation.lng,
              20000
            );
            
            const osm = await Promise.race([osmPromise, timeoutPromise]);
            
            if (controller.signal.aborted) return;
            
            const localNames = localTheatres.map((c) => c.name.toLowerCase());
            const newOsmTheatres = [];
            ...
            if (newOsmTheatres.length > 0 && !controller.signal.aborted) {
              const combined = [...localTheatres, ...newOsmTheatres];
              combined.sort((a, b) => (a.distance ?? 999) - (b.distance ?? 999));
              setTheatres(combined.slice(0, 10));
            }
          } catch (e) {
            console.warn('OSM fetch failed or timed out:', e.message);
          }
        }

        if (controller.signal.aborted) return;
        
        // If we still have no theatres, it's not an error but it's empty
        if (localTheatres.length === 0) {
           setTheatres([]);
        }
  ...
  }, [userLocation, selectedCity, retryCount]);
    ...
    return (
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Nearby Theatres</h2>
            <p className="text-xs text-gray-500 mt-0.5">Unable to load nearby cinemas</p>
          </div>
        </div>
        <div className="glass rounded-2xl p-6 text-center border-red-500/20">
          <p className="text-sm text-red-400 mb-4">Could not load nearby theatres.</p>
          <button 
